...
class PANUDYAMMSMEController {
  static async verifyPANUDYAMMSMEStatus(req, res) {
    let documentType
    try {
  ...
  static async verifyPANUDYAMMSMEStatusTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.pan_udyam_msme_status.success.data
        : MOCK_RESPONSES.pan_udyam_msme_status.failure.data
...
module.exports = PANUDYAMMSMEController
