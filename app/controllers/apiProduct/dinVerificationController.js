...
class DirectorVerification {
  static async verifyDIN(req, res) {
    let documentType
    try {
  ...
  static async verifyDINTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.din_verification.success.data
        : MOCK_RESPONSES.din_verification.failure.data
...
module.exports = DirectorVerification
