...
class CorporateGSTINController {
  static async verifyGSTIN(req, res) {
    try {
      ...
      try {
        ...
        return result.success
          ? ResponseHelper.success(
              res,
              result.data,
              'Corporate GSTIN verification successful'
            )
          : ResponseHelper.error(
              res,
              'Corporate GSTIN verification failed',
              400,
              result.error
            )
      } catch (error) {
  ...
  static async verifyGSTINTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.corporate_gstin.success.data
        : MOCK_RESPONSES.corporate_gstin.failure.data
...
module.exports = CorporateGSTINController
