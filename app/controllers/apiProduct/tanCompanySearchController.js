...
class TanCompanySearchController {
  static async searchCompanyTan(req, res) {
    try {
      ...
      try {
        ...
        return result.success
          ? ResponseHelper.success(
              res,
              result.data,
              'Fecthed Details For Company Search'
            )
          : ResponseHelper.error(res, 'No Record Found', 400, result.error)
      } catch (error) {
  ...
  static async searchCompanyTanTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.tan_search.success.data
        : MOCK_RESPONSES.tan_search.failure.data
...
module.exports = TanCompanySearchController
