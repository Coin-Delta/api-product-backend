...
class GSTDetailsAdvanceController {
  static async verifyGSTAdvance(req, res) {
    let documentType
    try {
  ...
  static async verifyGSTAdvanceTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.gst_details_advance.success.data
        : MOCK_RESPONSES.gst_details_advance.failure.data
...
module.exports = GSTDetailsAdvanceController
