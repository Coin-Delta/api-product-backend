...
class rationCardVerificationController {
  static async verifyRationCard(req, res) {
    let documentType
    try {
  ...
  static async verifyRationCardTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.ration_card.success.data
        : MOCK_RESPONSES.ration_card.failure.data
...
module.exports = rationCardVerificationController
