...
class CreditReportController {
  static async verifyCreditReport(req, res) {
    try {
      ...
      try {
        ...
        return result.success
          ? ResponseHelper.success(
              res,
              result.data,
              'Credit Report verification successful'
            )
          : ResponseHelper.error(
              res,
              'Credit Report verification failed',
              400,
              result.error
            )
      } catch (error) {
  ...
  static async verifyCreditReportTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.credit_report.success.data
        : MOCK_RESPONSES.credit_report.failure.data
...
module.exports = CreditReportController
