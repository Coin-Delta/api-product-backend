...
class TelecomVerifcationController {
  static async verifyTelecomDetails(req, res) {
    try {
      ...
      try {
        ...
        return result.success
          ? ResponseHelper.success(
              res,
              result.data,
              'Telecom Verification successful'
            )
          : ResponseHelper.error(
              res,
              'Telecom verification failed',
              400,
              result.error
            )
      } catch (error) {
  ...
  static async verifyTelecomDetailsTest(req, res) {
    try {
      const { documentData } = req.body

      // Return success or failure mock response based on whether documentData is provided
      const mockResponse = documentData
        ? MOCK_RESPONSES.telecom_verification.success.data
        : MOCK_RESPONSES.telecom_verification.failure.data
...
module.exports = TelecomVerifcationController
